...
pfQuest:SetScript("OnUpdate", function()
  ...
  if this.updateQuestGivers == true then
    if not this.givertick or this.givertick < GetTime() then
  ...
  for id, entry in pairs(this.queue) do
    ...
    -- track whether this was a structural change (new/remove) vs objective progress
  ...
  -- trigger questgiver update only on structural changes (quest added/removed)
  -- not on every objective progress RELOAD
  if tsize(this.queue) == 0 then
    this.updateQuestLog = true
    if this.structuralChange then
      this.updateQuestGivers = true
      this.structuralChange = nil
    end
  end
end)
